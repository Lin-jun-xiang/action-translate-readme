...
* Nous savons tous que rédiger des documents README est chronophage, mais il existe maintenant une solution pour vous faire gagner la moitié du temps. C'est notre `action-translate-readme`

* Traduire différentes versions linguistiques de README via **l'IA générative**
...
> [!IMPORTANT]
> Comme les résultats de la traduction par le modèle d'IA générative peuvent parfois présenter des problèmes, il est recommandé d'effectuer les opérations sur une branche et de fusionner ensuite avec la branche principale.
...
3. Choisissez le modèle de traduction GPT (**optionnel**)
   ...
   * `zhipuai` : Si vous avez besoin d'une traduction **gratuite et stable**, vous pouvez vous inscrire sur la [Plateforme Zhipuai AI](https://open.bigmodel.cn/dev/howuse/introduction) et [demander un API KEY](https://open.bigmodel.cn/usercenter/proj-mgmt/apikeys), qui est un modèle GPT completely gratuit sans nécessiter de carte de crédit.
     * Pour utiliser cette option, veuillez suivre la méthode d'ajout de GitHub Token à **l'étape 2-2**, et ajouter **API KEY de Zhipuai AI aux Secrets de GitHub Repos**.
   ...
   * `openai` : **Garantit une qualité élevée et stable**, si vous avez un OPENAI API KEY, `gpt-4o` sera utilisé comme modèle de traduction.
     * Pour utiliser cette option, veuillez suivre la méthode d'ajout de GitHub Token à **l'étape 2-2**, et ajouter **API KEY d'OpenAI aux Secrets de GitHub Repos**.
...
4. Créez votre exemple d'action dans `.github/workflows/your_action.yml`. Vous pouvez directement copier ce qui suit :
    ...
    * `zhipuai` : API Zhipuai, ajoutée comme indiqué à l'étape trois (optionnel)
    * `openai` : API OpenAI, ajoutée comme indiqué à l'étape trois (optionnel)
